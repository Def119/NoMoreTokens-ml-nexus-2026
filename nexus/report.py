...
def report(out, learning=True):
    ...
    masked=np.full(len(tr),np.nan);importance=[];local=[];curves=[]
    ...
    for f in sorted(oof.fold.unique()):
        ...
        if learning:
            ...
            family=winner if winner in FAMILIES else s['folds'][int(f)]['best_inner_family']
            config=bundle['chosen'][family]
            fitted=bundle['models'][config['name']]
            for fraction in [.5,.75,1.]:
                ...
                with threadpool_limits(limits=4):
                    model=fitted if fraction==1 else fit_model(config,tr.iloc[chosen],y[chosen],seed=20260913+int(f)*100,
...
def make_trust_card(out,e,subgroup,imp):
    ...
    lines=[
        ...
        'Five stratified outer folds (seed 20260913), each with three inner folds. Preprocessing, candidate selection, stopping iterations and ensemble fitting are restricted to the outer training partition. Outer labels are used for scoring only. Choosing a final recipe from several outer scores still introduces selection optimism; historical development also used these labels. This is not a pristine external validation dataset.',
        ...
        '\nRun: `python -u -m nexus.train --output runs/nested_v7 --config runs/nested_v7/resolved_config.json --resume`.',
...
def make_notebook(out):
    ...
    nb.cells=[nbformat.v4.new_markdown_cell('# ML & AI Nexus 2026: reproducible refinement\n\nThis notebook audits saved experimental evidence and supplies the full rerun command. Execute from the repository root using the project virtual environment. No external data or pretrained weights are used.'),
              ...
              nbformat.v4.new_code_cell("RUN_TRAINING = False\nif RUN_TRAINING:\n    import subprocess, sys\n    subprocess.run([sys.executable, '-u', '-m', 'nexus.train', '--config', str(RUN / 'resolved_config.json'), '--output', 'runs/reproduction', '--deadline', ''], cwd=ROOT, check=True)")]
